...
public enum PopupCustomOption {
    case layout(FHRPopupViewController.PopupLayout)
    case animation(FHRPopupViewController.PopupAnimation)
    case backgroundStyle(FHRPopupViewController.PopupBackgroundStyle)
...
public protocol PopupContentViewController {
    ...
    /** sizeForPopup(popupController: size: showingKeyboard:):
     return view's size
     */
    func sizeForPopup(_ popupController: FHRPopupViewController, size: CGSize, showingKeyboard: Bool) -> CGSize
}

open class FHRPopupViewController: UIViewController {
    public enum PopupLayout {
        ...
        func origin(_ view: UIView, size: CGSize = UIScreen.main.bounds.size) -> CGPoint {
            ...
            case .center: return CGPoint(x: (size.width - view.frame.width) / 2, y: ((size.height - view.frame.height) / 2) - 44)
    ...
    fileprivate var closedHandler: ((FHRPopupViewController) -> Void)?
    fileprivate var showedHandler: ((FHRPopupViewController) -> Void)?
...
// MARK: - Publics
public extension FHRPopupViewController {

    // MARK: Classes
    class func create(_ parentViewController: UIViewController) -> FHRPopupViewController {
        let controller = FHRPopupViewController()
    ...
    func customize(_ options: [PopupCustomOption]) -> FHRPopupViewController {
        customOptions(options)
        return self
    }

    @discardableResult
    func show(_ childViewController: UIViewController,_ layout: PopupLayout = .center) -> FHRPopupViewController {
    ...
    func didShowHandler(_ handler: @escaping (FHRPopupViewController) -> Void) -> FHRPopupViewController {
        self.showedHandler = handler
        return self
    }

    func didCloseHandler(_ handler: @escaping (FHRPopupViewController) -> Void) -> FHRPopupViewController {
...
// MARK: Privates
private extension FHRPopupViewController {
    ...
    func registerNotification() {
        NotificationCenter.default.addObserver(self, selector: #selector(FHRPopupViewController.popupControllerWillShowKeyboard(_:)), name:
            UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(FHRPopupViewController.popupControllerWillHideKeyboard(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(FHRPopupViewController.popupControllerDidHideKeyboard(_:)), name: UIResponder.keyboardDidHideNotification, object: nil)
    }
    ...
    func registerTapGesture() {
        let gestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(FHRPopupViewController.didTapGesture(_:)))
...
extension FHRPopupViewController: UIScrollViewDelegate {
...
extension FHRPopupViewController: UIGestureRecognizerDelegate {
